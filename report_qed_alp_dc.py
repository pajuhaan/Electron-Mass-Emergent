...
DCQEDALP(alpha), reconstructed from retained QED coefficients through the
ALP bridge in the alpha paper.  Path B is not modified, because Path B uses the
bulk vector overlap ζ_B in its Ward factors rather than the scalar DC block.
"""
...
def compute_rlvm_with_qed_alp_scalar(run, alpha: float = CONSTANTS.α):
    """Compute Path A with the QED+ALP scalar audit branch in the scalar count only."""
    branch = build_qed_alp_branch(alpha, order=5)
    alpha_blocks_qed = replace(run.alpha_blocks, α=alpha, DC=branch.DC_qed_alp)
...
def alpha_fixed_point_map_qed_alp(alpha_unknown: float, run) -> float:
    ...
    # Path B remains vector-channel only.  The bulk vector representative ζ_B
...
def main() -> None:
    ...
    print_header(
        ...
        "Separate report; baseline Relator DC(α) path is not modified. Author: Mehrdad Pajuhaan",
    )
    console.print(constants_table(CONSTANTS))

    run = run_baseline()
    branch_input, rlvm_qed = compute_rlvm_with_qed_alp_scalar(run, CONSTANTS.α)
    ...
    coeff_rows = [(f"c{idx}_QED", format_float(coeff, 18), "dimensionless", "DCQEDALP coefficient") for idx, coeff in enumerate(C_QED, start=1)]
    ...
    scalar_rows = [
        ...
        ("DCQEDALP(α)", format_float(branch_input.DC_qed_alp, 18), "dimensionless", "DCQEDALP(alpha)"),
        ("Dstar target", format_float(Dstar_QED_ALP_target, 18), "dimensionless", "fixed alpha-paper reduced target"),
        ("DCQEDALP-D_star", format_float(branch_input.target_residual, 18), "dimensionless", "target residual at run alpha"),
        ("alpha from DCQEDALP=D_star", format_float(alpha_qed_target, 18), "dimensionless", "inverse-alpha diagnostic in alpha paper"),
    ...
    mass_rows = []
    for case, m in (
        ("Path A baseline DC", run.rlvm.m_A_MeV),
        ("Path A with DCQEDALP", rlvm_qed.m_A_MeV),
    ...
    mass_rows.append(("A_{DCQEDALP} - B", "", format_float(eV_from_MeV(rlvm_qed.m_A_MeV - run.rltm.m_B_MeV), 16), ""))
    console.print(rich_table("Electron-mass rows", ("Case", "m c² [MeV]", "Δ vs ref [eV]", "Δ vs ref [ppm]"), mass_rows))

    alpha_rows = [
        ("Path equality alpha, DC", format_float(alpha_path_eq_baseline, 18), f"relative {format_float(ppb((alpha_path_eq_baseline-CONSTANTS.α)/CONSTANTS.α), 14)} ppb vs run input"),
        ("Path equality alpha, DCQEDALP", format_float(alpha_path_eq, 18), f"relative {format_float(ppb((alpha_path_eq-CONSTANTS.α)/CONSTANTS.α), 14)} ppb vs run input"),
    ...
    audit_rows = [
        ...
        ("QED+ALP diagnostic", "separate", "does not modify DC(α), RLVM baseline, RLTM baseline, or lepton hierarchy"),
    ]
    console.print(rich_table("No-contamination audit", ("Check", "Status", "Meaning"), audit_rows))

    channel_rows = [
        ("Path A scalar slot", "DC(α) or DCQEDALP(alpha)", "N_eff_eff,A = N_eff_raw,A [1 + rho*² scalar DC]"),
        ("Path A local EM slot", "ζ_soft", "ring-local Lambda_ind only"),
        ("Path B vector slot", "ζ_B", "finite Ward factors only"),
